...
knitr::opts_knit$set(root.dir = "D:/magistrale/OMERF/data")
```
...
```{r}
om.0 = omerf_clm(y, cov[, -which(names(cov) %in% c("ESCS"))], group, toll=0.1)
summary.omerf_clm(om.0)
om.0$clmm.model$ST
VarCorr(om.0$clmm.model)

om.1 = omerf_clm(y, cov, group, toll=0.1)
summary.omerf_clm(om.1)
om.1$clmm.model$ST
...
# TEST
y_omerf_clm_t.0 = predict.omerf_clm(om.0, y, data_test, data_test$group, type='response')
mu_omerf_clm_t.0=predict.omerf_clm(om.0, y, data_test, data_test$group, type='mu')
max_mu_omerf_clm_t.0 = apply(mu_omerf_clm_t.0, 1, max)
eta_omerf_clm_t.0=predict.omerf_clm(om.0, y, data_test, data_test$group, type='eta')

y_omerf_clm_t.1 = predict.omerf_clm(om.1, y, data_test, data_test$group, type='response')
mu_omerf_clm_t.1=predict.omerf_clm(om.1, y, data_test, data_test$group, type='mu')
max_mu_omerf_clm_t.1 = apply(mu_omerf_clm_t.1, 1, max)
eta_omerf_clm_t.1=predict.omerf_clm(om.1, y, data_test, data_test$group, type='eta')
```

Random Effects - OMERF

```{r}
ranef.omerf_clm(om.0,group)
ord.re <- order(om.0$clmm.model$ranef)
ord.re

ranef.omerf_clm(om.1,group)
...
colnames(results.acc)=c('clm', 'clmm','ordforest', 'omerf_clm')
colnames(results.mse)=c('clm', 'clmm','ordforest', 'omerf_clm')
colnames(results.oc)=c('clm', 'clmm','ordforest', 'omerf_clm')
colnames(results.ari)=c('clm', 'clmm','ordforest', 'omerf_clm')
colnames(results.ck)=c('clm', 'clmm','ordforest', 'omerf_clm')
colnames(results.newi)=c('clm', 'clmm','ordforest', 'omerf_clm')
...
results.acc[1,4]=sum(y_test==as.numeric(y_omerf_clm_t.0))/length(y_test)
results.mse[1,4]=mean((y_test - as.numeric(y_omerf_clm_t.0))^2)
cm_omerf_clm <- table(y_test, as.numeric(y_omerf_clm_t.0))
cm_omerf_clm
num_classes_omerf_clm <- max(nrow(cm_omerf_clm), ncol(cm_omerf_clm))
cm_omerf_clm2 <- matrix(0, nrow = num_classes_omerf_clm, ncol = num_classes_omerf_clm)
rownames(cm_omerf_clm2) <- colnames(cm_omerf_clm2) <- lev
cm_omerf_clm2[rownames(cm_omerf_clm), colnames(cm_omerf_clm)] <- cm_omerf_clm[rownames(cm_omerf_clm), colnames(cm_omerf_clm)]
results.oc[1,4]=OrdinalClassificationIndex(cm_omerf_clm2, dim(cm_omerf_clm2)[1])
results.ari[1,4]=adj.rand.index(y_test, as.numeric(y_omerf_clm_t.0))
results.ck[1,4]=cohen.kappa(x=cbind(y_test,as.numeric(y_omerf_clm_t.0)))$kappa
results.newi[1,4]=newindex(mu_omerf_clm_t.0,y_test_newi,num_classes_omerf_clm)$norm.index


results.acc[2,4]=sum(y_test==as.numeric(y_omerf_clm_t.1))/length(y_test)
results.mse[2,4]=mean((y_test - as.numeric(y_omerf_clm_t.1))^2)
cm_omerf_clm <- table(y_test, as.numeric(y_omerf_clm_t.1))
cm_omerf_clm
num_classes_omerf_clm <- max(nrow(cm_omerf_clm), ncol(cm_omerf_clm))
cm_omerf_clm2 <- matrix(0, nrow = num_classes_omerf_clm, ncol = num_classes_omerf_clm)
rownames(cm_omerf_clm2) <- colnames(cm_omerf_clm2) <- lev
cm_omerf_clm2[rownames(cm_omerf_clm), colnames(cm_omerf_clm)] <- cm_omerf_clm[rownames(cm_omerf_clm), colnames(cm_omerf_clm)]
results.oc[2,4]=OrdinalClassificationIndex(cm_omerf_clm2, dim(cm_omerf_clm2)[1])
results.ari[2,4]=adj.rand.index(y_test, as.numeric(y_omerf_clm_t.1))
results.ck[2,4]=cohen.kappa(x=cbind(y_test,as.numeric(y_omerf_clm_t.1)))$kappa
results.newi[2,4]=newindex(mu_omerf_clm_t.1,y_test_newi,num_classes_omerf_clm)$norm.index
