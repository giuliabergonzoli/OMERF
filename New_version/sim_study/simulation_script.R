...
for(nr in 1:nruns) {
  ...
  clmm.mod=clmm(y ~ x1+x2+x3+x4+x5+x6+x7+(1|gr), link='logit', data=for.data, Hess=TRUE, control=clmm.control(maxLineIter = 200, maxIter=500, grtol=1e-4))
  omerf.mod=omerf(y, cov, gr, toll=0.07)
...
# save the results
r=2
name.acc=paste('risultati_fin_acc_',r,'.txt', sep='')
name.mse=paste('risultati_fin_mse_',r,'.txt', sep='')
name.oc=paste('risultati_fin_oc_',r,'.txt', sep='')
name.ari=paste('risultati_fin_ari_',r,'.txt', sep='')
name.ck=paste('risultati_fin_ck_',r,'.txt', sep='')
name.newi=paste('risultati_fin_newi_',r,'.txt', sep='')
