...
for (nr in 1:nruns) {
  ...
  # OMERF (ranger, ordinal forest initialization) — only x1, x2, x3 — tracking
  t0              <- proc.time()["elapsed"]
  omerf.mod       <- omerf(y, cov[, c('x1', 'x2', 'x3')], gr, toll = 0.05)
  ...
  # OMERF_CLM (ranger, CLM initialization) — only x1, x2, x3
  t0                  <- proc.time()["elapsed"]
  omerf_clm.mod <- omerf_clm(y, cov[, c('x1', 'x2', 'x3')], gr, toll = 0.05)
