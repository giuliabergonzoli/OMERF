...
source('OMERF.R')
source('OMERF_clm.R')
source('build_dataset_slope_new.R')
...
for(nr in 1:nruns) {
  set.seed(nr)
  print(nr)
  # n=round(runif(1, min = 50, max = 100))#number of data to use (test+train)
  # train set preparation
  dati=build.dataset(nr,1,1,prop)
  ...
  omerf_clm.mod = omerf_clm(y, cov, gr, znam=c('x1')) 
