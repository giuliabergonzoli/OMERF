...
source('OMERF.R')
source('OMERF_clm.R')
source('build_dataset_lin_new.R')
...
for(nr in 1:nruns) {
  ...
  clm.mod=clm(y ~ x1+x2+x3+d2+d3+d4+d5+d6+d7+d8+d9+d10+d11+d12+d13+d14+d15 , data=clm.data, link='logit') #d1: base case
  ...
  omerf_clm.mod = omerf_clm(y, cov[,c('x1','x2','x3')], gr) 
